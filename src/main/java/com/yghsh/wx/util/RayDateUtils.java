...
public class RayDateUtils {
	...
	public static String searchDateAddOneDay(String date) {
		if (date == null || date.length() < 1)
			return null;
		return RayDateUtils.dateToStr(
				YYMMDD,
				RayDateUtils.addDays(
						RayDateUtils.strToDate(RayDateUtils.YYMMDD, date), 1));
	}
	 ...
	 * 
	 * @param dateStr
	 *            excel中表示时间的字符串 ，形式为 2013/5/1 或 41415
	 * @return
	 */
	public static Date getDateFromExcel(String dateStr) {
		...
		try {
			...
			Integer dateValue = Integer.valueOf(dateStr.substring(0,
					dateStr.lastIndexOf(".")));
